...
#define MT6835_CPR 2097152  // 2^21
#define MT6701_CPR 16384    // 2^14

// Auto-detected chip type
enum MagEncChipType { CHIP_UNKNOWN = 0, CHIP_MT6835, CHIP_MT6701 };
...
static SPISettings MT6835SPISettings(8000000, MT6835_BITORDER, SPI_MODE3); // 8MHz
...
public:
    ...
    // Encoder diagnostics (public for reporting)
    MagEncChipType chip_type = CHIP_UNKNOWN;
...
private:
	...
	int nCS = -1;
    uint32_t last_good_angle = 0;

    void transfer24(MT6835Command* outValue);
    uint8_t readRegister(uint16_t reg);
    bool writeRegister(uint16_t reg, uint8_t value);

    static uint8_t calcCRC8(uint8_t d0, uint8_t d1, uint8_t d2);
    static uint8_t calcCRC6(uint32_t data, int bits);
